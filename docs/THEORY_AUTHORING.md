...
- File name must match topic id exactly.
  - Example topic id: `promises`
  - File path: `src/content/theory/javascript/promises.ts`

## Add a new theory page

1. Create topic content file under `src/content/theory/javascript/`.
2. Export a `TopicTheoryContent` object.
3. Register the content in `THEORY_CONTENT_BY_TOPIC_ID` in `src/content/theory/index.ts`.
4. Run lint/build.

After registration:
- `/javascript/<topic-id>/theory` is statically generated.
