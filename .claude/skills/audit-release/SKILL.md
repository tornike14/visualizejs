...
Run `npm run typecheck`. The `VISUALIZATIONS`, theory, keyword, and description registries are typed by `TopicId`, so a topic missing from any of them fails here. Also confirm each topic id has a folder under `src/components/visualizations/`.

### 4. ExampleSelector Registration

For topics whose `index.tsx` does not use `ExamplePicker`, verify the registry entry sets `toolbar: "simple"` so the loading skeleton matches.
