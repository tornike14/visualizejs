...
const claims = (state: ClaimState): ClaimRow[] => [
  ...
  claim("role", '"user"', "custom claim the app reads for authorization", state),
...
const LOGIN_MSG = (status: FlowMessage["status"]) =>
  msg("login", "client", "server", "POST /login", status, "{ email, password }");

const ISSUE_CODE = [
  { num: 1, text: 'const { createHmac } = require("crypto");' },
  { num: 2, text: 'const b64url = (s) => Buffer.from(s).toString("base64url");' },
  { num: 3, text: "function sign(payload, secret) {" },
  { num: 4, text: '  const header = b64url(JSON.stringify({ alg: "HS256", typ: "JWT" }));' },
  { num: 5, text: "  const body = b64url(JSON.stringify(payload));" },
  { num: 6, text: '  const sig = createHmac("sha256", secret).update(`${header}.${body}`).digest("base64url");' },
  { num: 7, text: "  return `${header}.${body}.${sig}`;" },
  { num: 8, text: "}" },
  { num: 9, text: 'app.post("/login", async (req, res) => {' },
  { num: 10, text: "  const user = await db.users.findByEmail(req.body.email);" },
  { num: 11, text: "  if (!user || !(await bcrypt.compare(req.body.password, user.passwordHash))) {" },
  { num: 12, text: '    return res.status(401).json({ error: "invalid credentials" });' },
  { num: 13, text: "  }" },
  { num: 14, text: "  const now = Math.floor(Date.now() / 1000);" },
  { num: 15, text: "  const token = sign({ sub: String(user.id), role: user.role, iat: now, exp: now + 3600 }, SECRET);" },
...
const ISSUE_STEPS: JwtStep[] = [
  ...
  step({
    ...
    messages: [
      LOGIN_MSG("done"),
      msg("login-ok", "server", "client", "200 OK", "done", '{ token: "eyJhbGciOi..." }'),
    ],
    activeActorId: "client",
    claims: claims("neutral"),
    tokenParts: [header("encoded"), payload("encoded"), signature("encoded")],
...
const VERIFY_CODE = [
  ...
  { num: 3, text: '  const expected = createHmac("sha256", secret).update(`${header}.${body}`).digest("base64url");' },
  { num: 4, text: "  if (expected.length !== sig.length || !timingSafeEqual(Buffer.from(expected), Buffer.from(sig))) {" },
  { num: 5, text: '    throw new Error("invalid signature");' },
  { num: 6, text: "  }" },
  { num: 7, text: '  const payload = JSON.parse(Buffer.from(body, "base64url").toString());' },
  { num: 8, text: '  if (payload.exp <= Math.floor(Date.now() / 1000)) throw new Error("token expired");' },
  { num: 9, text: "  return payload;" },
  { num: 10, text: "}" },
  { num: 11, text: "function requireAuth(req, res, next) {" },
  { num: 12, text: '  const auth = req.headers.authorization ?? "";' },
  { num: 13, text: '  if (!auth.startsWith("Bearer ")) return res.status(401).json({ error: "missing token" });' },
  { num: 14, text: "  try {" },
  ...
  { num: 19, text: "  }" },
  { num: 20, text: "}" },
  { num: 21, text: 'app.get("/me", requireAuth, (req, res) => res.json({ id: req.user.sub, role: req.user.role }));' },
];
...
const VALID_MESSAGES = [
  ME_MSG("me-1", "done"),
  msg("me-1-ok", "server", "client", "200 OK", "done", '{ id: "42", role: "user" }'),
];

const TAMPERED_MESSAGES = [
  ...VALID_MESSAGES,
  ME_MSG("me-2", "done", "Bearer with role changed to admin"),
  msg("me-2-401", "server", "client", "401 Unauthorized", "failed", '{ error: "invalid signature" }'),
];
...
const VERIFY_STEPS: JwtStep[] = [
  ...
  step({
    ...
    messages: [...VALID_MESSAGES, ME_MSG("me-2", "active", "Bearer with role changed to admin")],
    activeActorId: "server",
    checks: checks("pass", "pending", "pending", "pending"),
  }),
  step({
  ...
  step({
    ...
    messages: [...TAMPERED_MESSAGES, ME_MSG("me-3", "active", "Bearer with a 3 hour old token")],
    activeActorId: "server",
    checks: checks("pass", "pass", "pending", "pending", {
      sig: "digest matches, constant-time compare",
    }),
  ...
  step({
    ...
    messages: [
      ...
      msg("me-3-401", "server", "client", "401 Unauthorized", "failed", '{ error: "token expired" }'),
    ],
    activeActorId: "client",
...
const REFRESH_CODE = [
  ...
  { num: 3, text: "  const row = await db.refreshTokens.findOne({ hash: sha256(presented) });" },
  { num: 4, text: "  if (!row || row.revokedAt || row.expiresAt < Date.now()) {" },
  { num: 5, text: '    return res.status(401).json({ error: "refresh token invalid" });' },
  { num: 6, text: "  }" },
  { num: 7, text: '  const next = randomBytes(32).toString("base64url");' },
  { num: 8, text: "  await db.refreshTokens.rotate(row.id, sha256(next));" },
  { num: 9, text: '  res.cookie("refresh_token", next, { httpOnly: true, secure: true, sameSite: "strict", path: "/refresh" });' },
  { num: 10, text: "  const now = Math.floor(Date.now() / 1000);" },
  { num: 11, text: "  res.json({ token: sign({ sub: row.userId, role: row.role, iat: now, exp: now + 900 }, SECRET) });" },
  { num: 12, text: "});" },
  { num: 13, text: 'app.post("/logout", async (req, res) => {' },
  { num: 14, text: "  await db.refreshTokens.revoke(sha256(req.cookies.refresh_token));" },
  { num: 15, text: '  res.clearCookie("refresh_token", { path: "/refresh" });' },
  { num: 16, text: "  res.status(204).end();" },
  { num: 17, text: "});" },
];

const ACCESS = (value: number, display: string, active = false) =>
  bar("access", "access", value, display, value === 0 ? "rose" : "cyan", active);
const REFRESH = (value: number, display: string, active = false) =>
  bar("refresh", "refresh", value, display, value === 0 ? "rose" : "violet", active);
...
const ME_OK = msg("me-ok", "client", "server", "GET /me", "done", "200, no DB lookup");
const ME_EXPIRED = msg("me-exp", "server", "client", "401 token expired", "failed", "access exp 1700000900 passed");
const REFRESH_REQ = (status: FlowMessage["status"]) =>
  msg("refresh", "client", "server", "POST /refresh", status, "Cookie: refresh_token=...");
const DB_FIND = (status: FlowMessage["status"]) =>
  msg("db-find", "server", "db", "findOne", status, "hash = sha256(presented)");
const DB_ROW = msg("db-row", "db", "server", "row", "done", "user 42, not revoked");
const DB_ROTATE = msg("db-rotate", "server", "db", "rotate", "done", "replace hash, keep row");
const REFRESH_OK = msg("refresh-ok", "server", "client", "200 OK", "done", "new access + Set-Cookie");
const ME_OK_2 = msg("me-ok-2", "client", "server", "GET /me", "done", "200 with the new token");
const LOGOUT_REQ = (status: FlowMessage["status"]) =>
  msg("logout", "client", "server", "POST /logout", status, "Cookie: refresh_token=...");
const DB_REVOKE = msg("db-revoke", "server", "db", "revoke", "done", "set revokedAt");
const LOGOUT_OK = msg("logout-ok", "server", "client", "204 No Content", "done", "cookie cleared");
...
const REFRESH_STEPS: JwtStep[] = [
  ...
  step({
    ...
    doneLines: [1, 2],
    lifetimes: [ACCESS(0, "expired"), REFRESH(0.9996, "29d 23h", true)],
    messages: [LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("active")],
    activeActorId: "db",
  ...
  step({
    ...
    doneLines: [1, 2, 3],
    lifetimes: [ACCESS(0, "expired"), REFRESH(0.9996, "29d 23h", true)],
    messages: [LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("done"), DB_ROW],
    activeActorId: "server",
    store: [entry("rt1", RT1, "active", true)],
  }),
  step({
    ...
    messages: [
      LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("done"), DB_ROW, DB_ROTATE,
    ],
    activeActorId: "db",
  ...
  step({
    ...
    messages: [
      LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("done"), DB_ROW, DB_ROTATE, REFRESH_OK,
    ],
    activeActorId: "client",
    store: [entry("rt1", RT1, "rotated"), entry("rt2", RT2, "active")],
  }),
  step({
    ...
    messages: [
      LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("done"), DB_ROW, DB_ROTATE, REFRESH_OK, ME_OK_2,
    ],
    activeActorId: "server",
    store: [entry("rt1", RT1, "rotated"), entry("rt2", RT2, "active")],
  }),
  ...
  step({
    ...
    messages: [
      LOGIN_DONE, ME_OK, ME_EXPIRED, REFRESH_REQ("done"), DB_FIND("done"), DB_ROW, DB_ROTATE, REFRESH_OK, ME_OK_2,
      LOGOUT_REQ("done"), DB_REVOKE, LOGOUT_OK,
    ],
    activeActorId: "client",
