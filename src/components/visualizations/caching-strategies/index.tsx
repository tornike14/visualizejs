  ...
  const flashes = useChangeFlash(
    {
      ...
      cache: [currentStep?.cache, currentStep?.database],
      messages: [currentStep?.messages, currentStep?.activeActorId],
      stats: currentStep?.stats,
      queue: currentStep?.queue,
      recency: currentStep?.recency,
    },
    currentStepIndex,
  );
    ...
    <>
      ...
      <section className="relative flex flex-col gap-4 px-1 py-2 text-slate-100 sm:px-2 sm:py-3 lg:px-3 lg:py-4">
        <div className="grid gap-4 xl:grid-cols-[auto_minmax(0,1fr)]">
          ...
          <StatePanels kind={example.kind} step={currentStep} flashes={flashes} />
        </div>
      </section>
    </>
  );
