...
/* ── tick builders ── */
const ev = (t: number, label?: string): TimelineTick => ({
  t,
  state: "event",
  label,
});
const ign = (t: number, label?: string): TimelineTick => ({
  t,
  state: "ignored",
  label,
});
const pend = (t: number, label?: string): TimelineTick => ({
  t,
  state: "pending",
  label,
});
const canc = (t: number, label?: string): TimelineTick => ({
  t,
  state: "cancelled",
  label,
});
const call = (t: number, label?: string): TimelineTick => ({
  t,
  state: "call",
  label,
});
const trail = (t: number, label?: string): TimelineTick => ({
  t,
  state: "trailing",
  label,
});
...
const keyEvents = (count: number) =>
  KEYS.slice(0, count).map((k) => ev(k.t, `"${k.v}"`));
...
const closure = (
  timeoutId: string,
  lastArgs: string,
  tone: StateEntry["tone"] = "active",
): StateEntry[] => [
  {
    key: "timeoutId",
    value: timeoutId,
    tone: timeoutId === "null" ? NONE : tone,
  },
...
const DEBOUNCE_STEPS: DebounceThrottleStep[] = [
  ...
  D(
    ...
    5,
    [1, 2, 3, 4, 12, 13, 14, 15],
    debounceTimeline(0, 1, [pend(300, "timer 1")], {
      start: 0,
      end: 300,
      label: "wait",
    }),
  ...
  D(
    ...
    5,
    [1, 2, 3, 4, 12, 13, 14, 15],
    debounceTimeline(80, 2, [canc(300, "timer 1"), pend(380, "timer 2")], {
      start: 80,
      end: 380,
      label: "wait",
    }),
  ...
  D(
    ...
    debounceTimeline(
      160,
      3,
      [canc(300, "timer 1"), canc(380, "timer 2"), pend(460, "timer 3")],
      { start: 160, end: 460, label: "wait" },
    ),
    timer(3, 460, 160),
    closure("3", `["rea"]`),
  ),
  ...
  D(
    `Five keyups produced one call. This only works because the closure outlives every event. A debounced function created inside a React render body gets a new closure each render, so a keyup after a re-render cannot cancel the previous render's timer and both fetches go out; create it once with <code>useMemo</code> or <code>useRef</code>.`,
    null,
  ...
  rows: [
    {
      id: "events",
      label: "scroll",
      ticks: now === null ? [] : scrollEvents(upTo, calledAt),
    },
...
const throttleState = (
  ...
  {
    key: "inThrottle",
    value: String(inThrottle),
    tone: inThrottle ? "pending" : NONE,
  },
  {
    key: "pendingArgs",
    value: pendingArgs,
    tone: pendingArgs === "null" ? NONE : trailing ? "active" : "muted",
  },
  {
    key: "trailing",
    value: String(trailing),
    tone: trailing ? "success" : NONE,
  },
];
...
const THROTTLE_STEPS: DebounceThrottleStep[] = [
  ...
  T(
    ...
    throttleTimeline(150, 150, LEAD, CALLS_0, {
      start: 0,
      end: 200,
      label: "window",
    }),
  ...
  T(
    ...
    throttleTimeline(200, 200, LEAD, CALLS_200, {
      start: 200,
      end: 400,
      label: "window",
    }),
  ...
  T(
    ...
    throttleTimeline(350, 350, LEAD, CALLS_200, {
      start: 200,
      end: 400,
      label: "window",
    }),
  ...
  T(
    ...
    17,
    [...SETUP_DONE, 4, 5, 6, 7, 8, 15, 16],
    throttleTimeline(400, 400, LEAD, CALLS_400, {
      start: 400,
      end: 600,
      label: "window",
    }),
  ...
  T(
    ...
    16,
    [...SETUP_DONE, 4, 5, 6, 7, 8, 15, 17],
    throttleTimeline(500, 500, LEAD, CALLS_400, {
      start: 400,
      end: 600,
      label: "window",
    }),
  ...
  T(
    ...
    16,
    [...SETUP_DONE, 4, 5, 6, 7, 15, 17],
    throttleTimeline(150, 150, [0], CALLS_0, {
      start: 0,
      end: 200,
      label: "window",
    }),
  ...
  T(
    ...
    throttleTimeline(200, 200, [0], [...CALLS_0, trail(200, "y=300")], {
      start: 200,
      end: 400,
      label: "window",
    }),
  ...
  rows: [
    {
      ...
      label: "input",
      ticks:
        now === null ? [] : BURST.filter((t) => t <= upTo).map((t) => ev(t)),
    },
...
const throttleEvents = (upTo: number) =>
  BURST.filter((t) => t <= upTo).map((t) =>
    t === 0 || t === 500 ? call(t) : ign(t),
  );

const ROW_TIMING: ComparisonRow = {
  aspect: "fires",
  debounce: "after events stop",
  throttle: "at most once per window",
};
const ROW_FIRST: ComparisonRow = {
  aspect: "first event",
  debounce: "delayed by wait",
  throttle: "immediate (leading)",
};
const ROW_ARGS: ComparisonRow = {
  aspect: "args used",
  debounce: "last invocation",
  throttle: "first, or last with trailing",
};
const ROW_USE: ComparisonRow = {
  aspect: "use for",
  debounce: "search, resize end, autosave",
  throttle: "scroll, mousemove, analytics",
};
const ROW_CTRL: ComparisonRow = {
  aspect: "controls",
...
const COMPARE_STEPS: DebounceThrottleStep[] = [
  ...
  C(
    `One rule for both: create the wrapper once. A wrapper built inside a render function or an inline listener gets a fresh closure every time, so it cannot cancel or count the timers its predecessors started, and the limit silently stops working. Keep it in module scope, <code>useMemo</code>, or <code>useRef</code>.`,
    null,
...
export const EXAMPLES: DebounceThrottleExample[] = [
  {
    ...
    codeLines: [
      ...
      {
        num: 15,
        text: "input.addEventListener('keyup', (e) => search(e.target.value));",
      },
    ],
    panelDefs: [
  ...
  },
  {
    ...
    codeLines: [
      ...
      {
        num: 20,
        text: "const onScroll = throttle((y) => console.log('scrollY', y), 200);",
      },
  ...
  },
  {
    ...
    codeLines: [
      ...
      {
        num: 6,
        text: "window.addEventListener('resize', debounce(relayout, 150));",
      },
      { num: 7, text: "" },
      ...
      {
        num: 10,
        text: "window.addEventListener('mousemove', throttle(track, 100));",
      },
      { num: 11, text: "" },
      { num: 12, text: "// lodash options and controls" },
      {
        num: 13,
        text: "const save = _.debounce(persist, 1000, { maxWait: 5000 });",
      },
      { num: 14, text: "save.cancel(); // drop the pending call" },
      { num: 15, text: "save.flush();  // run the pending call now" },
      {
        num: 16,
        text: "_.throttle(fn, 200, { leading: true, trailing: false });",
      },
    ],
    panelDefs: [],
    steps: COMPARE_STEPS,
