...
function getLineRange(node: Node): number[] {
  if (!node.loc) return [];
  const lines: number[] = [];
...
function expressionToLabel(node: Node, source: string): string {
  if (!node.loc) return source.slice(node.start, node.end);
  const raw = source.slice(node.start, node.end);
  if (raw.length <= 45) return raw;
  return raw.slice(0, 42) + "...";
}

function extractConsoleLogArg(node: CallExpression, source: string): string | null {
  const callee = node.callee;
  if (
    callee.type === "MemberExpression" &&
    (callee.object as Identifier).name === "console" &&
    (callee.property as Identifier).name === "log"
  ) {
    ...
    if (arg.type === "Literal") {
      return String((arg as Literal).value);
    }
    if (arg.type === "Identifier") {
      return (arg as Identifier).name;
    }
...
function getCallbackBody(node: Node): Node[] | null {
  if (
    ...
    // Expression body arrow: () => expr — wrap in synthetic ExpressionStatement-like object
    return [fn.body];
  }
  return null;
}

function callbackLabel(bodySource: string): string {
  const trimmed = bodySource.trim();
  if (trimmed.length <= 30) return `() => ${trimmed}`;
  return `() => ${trimmed.slice(0, 27)}...`;
}

function extractCallbackFromArg(
  arg: Node,
  source: string,
...
function processCallExpression(
  ...
  // console.log(...)
  const logArg = extractConsoleLogArg(expr, source);
  if (logArg !== null) {
    const label = `console.log('${logArg}')`;
    state.stack.push(label);
    state.consoleOutput.push(logArg);
    pushStep(state, {
      descriptionHtml: `<span class="hl-stack">Call Stack</span> executes <code>${label}</code>. Output: <strong>${logArg}</strong>.`,
      activeLine: line,
  ...
  if (isSetTimeout(expr)) {
    ...
    pushStep(state, {
      descriptionHtml: `<code>${stLabel}</code> is pushed to the <span class="hl-stack">Call Stack</span>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
  ...
  if (isQueueMicrotask(expr)) {
    ...
    pushStep(state, {
      descriptionHtml: `<code>${qmLabel}</code> is pushed to the <span class="hl-stack">Call Stack</span>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
  ...
  if (isPromiseResolveThen(expr)) {
    ...
    pushStep(state, {
      descriptionHtml: `<code>${promiseLabel}</code> is pushed to the <span class="hl-stack">Call Stack</span>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
  ...
  if (isUserFunctionCall(expr, state)) {
    ...
    pushStep(state, {
      descriptionHtml: `<code>${fnLabel}</code> is called and pushed to the <span class="hl-stack">Call Stack</span>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
    ...
    for (const stmt of fnDecl.body.body) {
      const err = processStatement(stmt, source, state, isInsideCallback);
      if (err) return err;
    }
  ...
  if (hasThenCall(expr)) {
    ...
    if (typo) {
      return {
        ...
        message: `Promise.${typo}() is not a function. Did you mean Promise.resolve()?`,
        line: line ?? undefined,
      };
    }
    return {
      type: "unsupported-pattern",
      message: `.then() is only supported on Promise.resolve() chains. Example: Promise.resolve().then(() => { ... })`,
      line: line ?? undefined,
    };
  }
...
function processStatement(
  ...
  if (node.type === "FunctionDeclaration") {
    ...
    if (decl.id) {
      ...
      state.stack.push(fnLabel);
      pushStep(state, {
        descriptionHtml: `<code>${fnLabel}</code> declaration is hoisted and registered.`,
        activeLine: line,
        loopActive: isInsideCallback,
        loopLabel: isInsideCallback ? "running" : "idle",
      });
  ...
  if (node.type === "ExpressionStatement") {
    ...
    if (expr.type === "CallExpression") {
      const result = processCallExpression(expr as CallExpression, node, source, state, isInsideCallback);
    ...
    state.stack.push(label);
    pushStep(state, {
      descriptionHtml: `<span class="hl-stack">Call Stack</span> executes <code>${label}</code>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
  ...
  if (node.type === "CallExpression") {
    const result = processCallExpression(node as CallExpression, node, source, state, isInsideCallback);
    if (result !== "unhandled") return result;
  ...
  if (node.type === "VariableDeclaration") {
    ...
    pushStep(state, {
      descriptionHtml: `<span class="hl-stack">Call Stack</span> executes <code>${shortLabel}</code>.`,
      activeLine: line,
      loopActive: isInsideCallback,
      loopLabel: isInsideCallback ? "running" : "idle",
    });
  ...
  return {
    type: "unsupported-pattern",
    message: `Unsupported statement: ${node.type}. The sandbox supports console.log(), setTimeout(), Promise.resolve().then(), queueMicrotask(), function declarations, and variable declarations.`,
    line: line ?? undefined,
  };
}
  ...
  while (
    ...
    while (state.microtaskQueue.length > 0 && safety < 80) {
      ...
      for (const stmt of micro.bodyStatements) {
        const err = processStatement(stmt, source, state, true);
        if (err) return err;
      }
...
export function generateEventLoopSteps(
  ...
  if (ast.body.length === 0) {
    return {
      ...
      error: {
        ...
        message: "No statements found. Write some code to visualize!",
      },
    };
  }
  ...
  let doneHtml = `<strong>Done!</strong> Output order: ${outputList || "(no output)"}.`;
  if (uncalledFunctions.length > 0) {
    const names = uncalledFunctions.map((n) => `<code>${n}()</code>`).join(", ");
